...
RSS_FEEDS = {
    ...
    "cnbc_investment":   "https://www.cnbcindonesia.com/investment/rss",
    "kontan_investasi":  "https://rss.kontan.co.id/category/investasi",
    "kontan_saham":      "https://rss.kontan.co.id/category/saham",
    "bisnis_market":     "https://market.bisnis.com/rss",
    "kompas_ekonomi":    "https://ekonomi.kompas.com/rss/articles.xml",
    "tempo_bisnis":      "https://bisnis.tempo.co/rss/20",
...
# ── Alert Thresholds ─────────────────────────────────────────────
PRICE_DROP_THRESHOLD     = -3.0   # alert kalau drop > 3%
PRICE_PUMP_THRESHOLD     = 5.0    # alert kalau naik > 5%
CRYPTO_DROP_THRESHOLD    = -7.0   # crypto lebih volatile
CRYPTO_PUMP_THRESHOLD    = 10.0   # crypto pump threshold
SENTIMENT_VERY_POSITIVE  = 0.5
SENTIMENT_VERY_NEGATIVE  = -0.5
VOLUME_SPIKE_MULTIPLIER  = 2.0    # alert kalau volume 2x average
...
def ensure_indices(client):
    if not client.indices.exists(INDEX_PRICES):
        client.indices.create(index=INDEX_PRICES, body={
        ...
        print(f"✅ Created index: {INDEX_PRICES}")

    if not client.indices.exists(INDEX_NEWS):
        client.indices.create(index=INDEX_NEWS, body={
            "settings": {"number_of_shards": 1, "number_of_replicas": 0},
            "mappings": {
                "properties": {
        ...
        print(f"✅ Created index: {INDEX_NEWS}")
...
def detect_related_tickers(text: str) -> list:
    """Detect which tickers are mentioned in news."""
...
def format_number(num: float, is_price: bool = True) -> str:
...
def fetch_stock_prices(**context):
    ...
    for ticker, name in STOCKS.items():
        try:
            ...
            print(f"{emoji} {ticker} ({name}): "
                  f"Rp{format_number(price)} | {change_pct:+.2f}% | {signal}")
            ...
            if change_pct <= PRICE_DROP_THRESHOLD:
                alerts.append({
                    ...
                    "message": (
                        ...
                        f"├ Harga: Rp{format_number(price)}\n"
                        f"├ Perubahan: *{change_pct:+.2f}%* "
                        f"({change_abs:+.0f})\n"
                        f"├ High: Rp{format_number(high)}\n"
                        f"├ Low:  Rp{format_number(low)}\n"
                        f"└ Signal: *{signal}* ⚠️"
                    )
                })
...
def fetch_crypto_prices(**context):
    ...
    for ticker, name in CRYPTO.items():
        try:
            ...
            emoji = "🟢" if change_pct >= 0 else "🔴"
            print(f"{emoji} {ticker} ({name}): "
                  f"${format_number(price)} | {change_pct:+.2f}% | {signal}")
    ...
    for source_name, feed_url in RSS_FEEDS.items():
        try:
            ...
            for entry in feed.entries[:15]:
                ...
                # Sentiment alert for related tickers
...
def load_to_opensearch(**context):
    ...
    pos_news    = sum(1 for n in news_results
                      if n["sentiment_label"] == "positive")
    neg_news    = sum(1 for n in news_results
                      if n["sentiment_label"] == "negative")
...
with DAG(
    ...
    # 3 sources parallel → load → alerts
